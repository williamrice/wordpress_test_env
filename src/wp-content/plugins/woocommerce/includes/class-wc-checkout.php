...
class WC_Checkout
{
	...
	{
		wc_add_notice("beginning of update session", 'error');
		...
		wc_add_notice("before array walk", 'error');

		array_walk($address_fields, array($this, 'set_customer_address_fields'), $data);
		wc_add_notice("before customer save", 'error');
	...
	{
		try {
			wc_add_notice("Process checkout started", 'error');
			wc_add_notice(implode($_POST), 'error');

			$nonce_value    = wc_get_var($_REQUEST['woocommerce-process-checkout-nonce'], wc_get_var($_REQUEST['_wpnonce'], '')); // phpcs:ignore
			wc_add_notice("after values nonce", 'error');
			...
			wc_set_time_limit(0);
			wc_add_notice("before checkout process action", 'error');
			do_action('woocommerce_before_checkout_process');
			wc_add_notice("after before checkout process action", 'error');
			...
			wc_add_notice("after checkout process", 'error');

			$errors      = new WP_Error();
			$posted_data = $this->get_posted_data();
			wc_add_notice("afer posted data fetch", 'error');

			// Update session for customer and totals.
			$this->update_session($posted_data);
			wc_add_notice("afer updated session", 'error');
			...
			wc_add_notice($e->getMessage() . " This is where exception is caught", 'error');
		}
